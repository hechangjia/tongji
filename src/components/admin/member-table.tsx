                ...
                <tr key={row.id} className="align-top text-slate-700 transition hover:bg-cyan-50/50">
                  ...
                  <td className="px-5 py-5">
                    <div className="space-y-3">
                      ...
                      {!isCurrentAdmin ? (
                        <form action={deleteMemberAction} className="space-y-3">
                          <input type="hidden" name="id" value={row.id} />
                          <p className="text-xs text-rose-500">
                            仅限没有历史数据的成员；有历史数据请改为停用。
                          </p>
                          <button
                            type="submit"
                            className="inline-flex h-10 items-center justify-center rounded-[16px] border border-rose-200 bg-rose-50 px-4 text-sm font-semibold text-rose-700 transition duration-200 hover:border-rose-300 hover:bg-rose-100"
                          >
                            删除成员
                          </button>
                        </form>
                      ) : (
                        <p className="text-xs text-slate-400">当前登录管理员不可删除</p>
                      )}
                    </div>
                  </td>
                </tr>
