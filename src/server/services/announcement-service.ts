...
function formatAnnouncementTimestamp(value: Date) {
...
export function isAnnouncementVisible(
  ...
  return (
    ...
    announcement.publishAt.getTime() <= now.getTime() &&
    (!announcement.expireAt || announcement.expireAt.getTime() >= now.getTime())
  );
}
...
export function sortVisibleAnnouncements<T extends AnnouncementLike>(
  ...
  return [...announcements]
    ...
    .sort((left, right) => {
      ...
      return right.publishAt.getTime() - left.publishAt.getTime();
    });
}
