...
describe("sales entry action", () => {
  ...
  test("returns a friendly error when the legacy daily form is blocked by identifier sales", async () => {
    saveSalesRecordForUserMock.mockRejectedValue(
      new Error("当天已经有识别码成交记录，请改用识别码工作台继续录单"),
    );

    const formData = new FormData();
    formData.set("saleDate", getTodaySaleDateValue());
    formData.set("count40", "1");
    formData.set("count60", "0");
    formData.set("remark", "旧模式补录");

    await expect(saveSalesEntryAction(undefined, formData)).resolves.toMatchObject({
      status: "error",
      message: "当天已经有识别码成交记录，请改用识别码工作台继续录单",
      values: {
        count40: "1",
        count60: "0",
        remark: "旧模式补录",
