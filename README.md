...
- 账号密码登录
- `/login` 成员自助注册，注册后默认创建并激活 `MEMBER`
- 全站现代化 UI 壳层与移动端抽屉导航
- `ADMIN` / `LEADER` / `MEMBER` 三角色权限控制与登录后自动分流
- 成员每日录入与同日更新
- 我的记录
- 日榜 / 总榜
- 小组管理与组长指派
...
当前这条分支已经完成校园电话卡新流程的 phase-1 基础建设：
...
- 角色固定为 `ADMIN` / `LEADER` / `MEMBER`
- 套餐固定为 `40` / `60`
- `/login` 只允许自助注册普通成员，组长由管理员指定
- 小组管理与成员管理都必须保证同一小组同一时间最多一个组长
- 组长必须挂在自己的小组下，取消组长身份或转组时要同步清理原小组的组长关联
...
- 默认 seed 不会直接创建组长账号；组长通过管理员在 `/admin/groups` 或 `/admin/members` 中从成员指定。
...
1. 登录系统
2. 打开 `/entry`
3. 查看系统自动生成的今日目标、自我趋势、最近提醒，以及识别码工作台概览
4. 优先在“识别码成交”主入口选择自己名下识别码，录入套餐类型，并选择“已分配线索”或手动填写 `QQ号 + 专业`
5. 系统会把单笔识别码成交同步进旧的当日 `40 / 60` 汇总，供后续结算继续使用
6. 如果当天还没有识别码成交记录，仍可使用下方旧版 `40 / 60` 日汇总录入作为过渡入口
7. 在 `/records` 查看历史记录，包括旧汇总与识别码成交明细
...
- 一旦某天已经存在识别码成交记录，旧版日汇总录入会拒绝写入，避免两套数据源冲突。
...
- `.env` 是本地文件，不应提交。
- 当前共享 Neon 库如果尚未同步最新 Prisma schema（含 `DailyTarget`、`MemberReminder`、`IdentifierCode`、`ProspectLead`、`IdentifierSale` 等表），`/entry`、`/admin/codes`、`/admin/insights` 等页面会直接报 Prisma 缺表错误；部署前必须先同步 schema。
